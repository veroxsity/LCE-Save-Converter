...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        if (args.Length < 1)
        {
            Console.WriteLine("Usage: LceWorldConverter <java_world_path> [output_dir] [--world-type <classic|small|medium|large|flat|flat-small|flat-medium|flat-large>] [--all-dimensions] [--copy-players] [--preserve-entities]");
            Console.WriteLine();
            ...
            Console.WriteLine("  --world-type     Unified world profile selector (recommended):");
            Console.WriteLine("                   classic, small, medium, large, flat, flat-small, flat-medium, flat-large");
            Console.WriteLine("                   (flat = classic size + flat generator)");
            Console.WriteLine("  --small-world    Use 64-chunk (1024 block) world size");
            Console.WriteLine("  --medium-world   Use 192-chunk (3072 block) world size");
            Console.WriteLine("  --large-world    Use 320-chunk (5120 block) world size");
            Console.WriteLine("  --flat-world     Force output level.dat generatorName to flat");
        ...
        if (!TryParseWorldSettings(args, out int xzSize, out string sizeLabel, out bool flatWorld, out string? worldParseError))
        {
            Console.Error.WriteLine($"Error: {worldParseError}");
            return;
        }
        ...
        Console.WriteLine($"World size:  {sizeLabel} ({xzSize} chunks / {xzSize * 16} blocks)");
        Console.WriteLine($"World type:  {(flatWorld ? "Flat" : "Default")}");
        Console.WriteLine();
        ...
        try
        {
            ...
            byte[] lceLevelDat = LevelDatConverter.Convert(javaLevelDat, spawnChunkX, spawnChunkZ, xzSize, flatWorld, estimatedSpawnY);
    ...
    static bool TryParseWorldSettings(
        string[] args,
        out int xzSize,
        ...
        out string? error)
    {
        ...
        if (worldTypeIndex >= 0)
        {
            ...
            if (worldTypeIndex + 1 >= args.Length || args[worldTypeIndex + 1].StartsWith("--"))
            {
                xzSize = ClassicWorldSize;
                sizeLabel = "Classic";
